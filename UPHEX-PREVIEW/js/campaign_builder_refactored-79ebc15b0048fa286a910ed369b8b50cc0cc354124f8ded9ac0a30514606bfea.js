(() => {
  ...
  // app/javascript/packs/campaign_builder_refactored.js
  init_esbuild_jquery_shim();
  ...
  // app/javascript/shared/media_selector.js
  init_esbuild_jquery_shim();
  var MediaSelector = class {
    constructor() {
      this.selectedClass = "border border-blue-500 ring-2 ring-blue-500";
      this.mediaSourceInputs = null;
      this.mediaBoxes = {
        template: ".media-box",
        library: ".ad-account-media-box",
        upload: ".upload-box",
        googleTemplate: ".media-box-google-launch",
        googleLibrary: ".ad-account-media-box-google-launch"
      };
    }
    // Initialize media selector
    init() {
      this.setupMediaSourceToggle();
      this.applyExistingSelections();
      this.setupImageSelection();
      this.handlePmaxCampaign();
    }
    // Setup media source radio button toggle
    setupMediaSourceToggle() {
      const mediaSourceInputs = document.querySelectorAll('input[name="media-source"]');
      const googleMediaSourceInputs = document.querySelectorAll('input[name="media-source-google-launch"]');
      mediaSourceInputs.forEach((input) => {
        input.addEventListener("change", (e) => this.handleMediaSourceChange(e));
      });
      googleMediaSourceInputs.forEach((input) => {
        input.addEventListener("change", (e) => this.handleGoogleMediaSourceChange(e));
      });
    }
    // Handle media source change
    handleMediaSourceChange(event) {
      const mediaType = event.target.value;
      Object.values(this.mediaBoxes).forEach((selector) => {
        const element = document.querySelector(selector);
        if (element) element.classList.add("hidden");
      });
      switch (mediaType) {
        case "library":
          this.showElement(this.mediaBoxes.library);
          break;
        case "template":
          this.showElement(this.mediaBoxes.template);
          break;
        case "upload":
          this.showElement(this.mediaBoxes.upload);
          break;
      }
      const formElement = document.querySelector(".cbj-form");
      if (formElement) {
        formElement.classList.remove("hidden");
      }
    }
    // Handle Google media source change
    handleGoogleMediaSourceChange(event) {
      const mediaType = event.target.value;
      if (mediaType === "library") {
        this.hideElement(this.mediaBoxes.googleTemplate);
        this.showElement(this.mediaBoxes.googleLibrary);
      } else if (mediaType === "template") {
        this.hideElement(this.mediaBoxes.googleLibrary);
        this.showElement(this.mediaBoxes.googleTemplate);
      }
    }
    // Apply selections to existing checked images
    applyExistingSelections() {
      const selectedImages = document.querySelectorAll('input[name="remote_campaign_builder_job[ad_images][]"]:checked');
      const defaultImages = document.querySelectorAll('input[name="remote_campaign_builder_job[default_images][]"]:checked');
      const googleTempImages = document.querySelectorAll('input[name="remote_campaign_builder_job[google_temp_images][]"]:checked');
      selectedImages.forEach((input) => {
        const container = input.closest(".check");
        if (container) container.classList.add(...this.selectedClass.split(" "));
      });
      defaultImages.forEach((input) => {
        const container = input.closest(".check");
        if (container) container.classList.add(...this.selectedClass.split(" "));
      });
      googleTempImages.forEach((input) => {
        const container = input.closest(".check");
        if (container) container.classList.add(...this.selectedClass.split(" "));
      });
      if (selectedImages.length > 0) {
        this.setMediaSource("library");
      } else if (defaultImages.length > 0) {
        this.setMediaSource("template");
      }
      if (googleTempImages.length > 0) {
        const googleTemplateChecked = document.querySelectorAll('.media-box-google-launch input[type="checkbox"]:checked');
        const googleLibraryChecked = document.querySelectorAll('.ad-account-media-box-google-launch input[type="checkbox"]:checked');
        if (googleTemplateChecked.length > 0) {
          this.setGoogleMediaSource("template");
        } else if (googleLibraryChecked.length > 0) {
          this.setGoogleMediaSource("library");
        }
      }
      if (selectedImages.length > 0 || defaultImages.length > 0 || googleTempImages.length > 0) {
        this.expandMediaSection();
      }
    }
    // Setup image selection click handlers
    setupImageSelection() {
      document.addEventListener("click", (e) => {
        const imageCheckbox = e.target.closest(".image-checkbox");
        if (imageCheckbox) {
          const container = imageCheckbox.closest(".check");
          if (container) {
            if (imageCheckbox.checked) {
              container.classList.add(...this.selectedClass.split(" "));
            } else {
              container.classList.remove(...this.selectedClass.split(" "));
            }
          }
        }
      });
    }
    // Set media source radio button
    setMediaSource(value) {
      const input = document.querySelector(`input[name="media-source"][value="${value}"]`);
      if (input) {
        input.checked = true;
        input.dispatchEvent(new Event("change"));
      }
    }
    // Set Google media source radio button
    setGoogleMediaSource(value) {
      const input = document.querySelector(`input[name="media-source-google-launch"][value="${value}"]`);
      if (input) {
        input.checked = true;
        input.dispatchEvent(new Event("change"));
      }
    ...
    expandMediaSection() {
      const toggleButton = document.querySelector(".media-box")?.closest(":has(.toggle-fields)")?.querySelector(".toggle-fields");
      if (toggleButton) {
        const fieldsSection = toggleButton.parentElement?.nextElementSibling;
        if (fieldsSection?.classList.contains("hidden")) {
          toggleButton.click();
        }
      }
    }
    // Handle Performance Max campaign special case
    handlePmaxCampaign() {
      const campaignType = document.getElementById("campaign-type");
      if (campaignType?.value === "pmax") {
        const toggleButton = document.querySelector(".custom-media .toggle-fields");
        if (toggleButton) {
          toggleButton.click();
        }
      }
  ...
  // app/javascript/shared/toggle_handler.js
  init_esbuild_jquery_shim();
  var ToggleHandler = class {
    constructor() {
      this.toggleButtons = [];
      this.translations = {
    ...
    init(options = {}) {
      ...
      if (window.I18n) {
        this.translations.expand = window.I18n.expand_section || this.translations.expand;
        this.translations.collapse = window.I18n.collapse_section || this.translations.collapse;
      }
      this.setupToggleButtons();
      this.setupCampaignAdsetToggle();
      this.setupActionMenus();
    }
    // Setup toggle buttons for expandable sections
    setupToggleButtons() {
      const toggleButtons = document.querySelectorAll(".toggle-fields");
    ...
    toggleSection(button) {
      ...
      this.updateToggleButtonText(button, !isHidden);
      if (!isHidden) {
        this.hideMediaBoxes();
      }
    }
    // Update toggle button text
    updateToggleButtonText(button, isCollapsed) {
      const text = isCollapsed ? this.translations.expand : this.translations.collapse;
      button.innerHTML = `
      <span class="text-blue-700 dark:text-blue-200 underline">${text}</span>
      &nbsp;<i class="fas fa-angle-down"></i>
    `;
    }
    // Hide media boxes when collapsing
    hideMediaBoxes() {
      const mediaBoxes = [".media-box", ".ad-account-media-box"];
      mediaBoxes.forEach((selector) => {
        const element = document.querySelector(selector);
        if (element) element.classList.add("hidden");
      });
    }
    // Setup campaign adset toggle (for campaign list)
    setupCampaignAdsetToggle() {
      const toggleButtons = document.querySelectorAll(".toggle-campaign-adset");
      toggleButtons.forEach((button) => {
        button.addEventListener("click", () => {
          const campaignId = button.id;
          const adsetRows = document.querySelectorAll(`.adset-row-${campaignId}`);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const isFacebookCampaign = document.querySelector('[name*="platform"][value="facebook"]') || window.location.href.includes("platform=facebook");
    ...
    window.submitForm = (url, formData) => {
      campaignFormHandler.formElement = formData;
      campaignFormHandler.formElement.action = url;
      campaignFormHandler.submitForm();
    };
    window.startPolling = (campaignBuilderId, locationId, agencyId, templateId, platform) => {
      const context = {
        agencyId,
        locationId,
        templateId,
      ...
      campaignPolling.startContextPolling(campaignBuilderId, context);
    };
  });
})();
