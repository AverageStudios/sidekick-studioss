...
import { connectWorkspacePipedriveOAuthProvider } from "@/lib/crm-integration";
...
export async function GET(request: NextRequest) {
  ...
  if (providerError) {
    return failRedirect(
      ...
      providerError === "access_denied"
        ? "Pipedrive connection was canceled before it finished."
        : "Pipedrive connection could not be completed.",
    );
  }
  ...
  if (
    !code ||
    !statePayload ||
    statePayload.provider !== "pipedrive" ||
    (state && stateCookie && state !== stateCookie) ||
    (providerCookie && providerCookie !== "pipedrive")
  ) {
    return failRedirect(request, "Pipedrive connection was canceled or expired. Please try again.");
  }
  ...
  try {
    await connectWorkspacePipedriveOAuthProvider({
      admin,
      workspaceId,
      userId: user.id,
      code,
    ...
    redirectUrl.searchParams.set("saved", "pipedrive connected");
    const response = NextResponse.redirect(redirectUrl);
    clearOauthCookies(response);
    return response;
  } catch (error) {
    logRouteError("pipedrive callback", error);
    return failRedirect(request, "Pipedrive connection failed. Please try again.");
  }
}
