...
function WorkspaceSaveButton() {
  const { pending } = useFormStatus();

  return (
    <Button type="submit" disabled={pending}>
      {pending ? "Saving..." : "Save workspace image"}
    </Button>
  );
}
...
async function createCroppedWorkspaceLogoDataUrl(imageSrc: string, pixelCrop: Area) {
  ...
  if (!context) {
    throw new Error("Workspace logo editor could not access the canvas context.");
  }
...
export function WorkspaceLogoField({
  ...
  const resolvedLogoUrl = useMemo(() => {
    ...
    return previewUrl || currentLogoUrl || null;
  }, [currentLogoUrl, previewUrl, removeLogo]);

  return (
    <>
      <div className="rounded-2xl border border-[var(--line)] bg-[var(--panel-strong)] p-4">
        <input type="hidden" name="removeWorkspaceLogo" value={removeLogo ? "1" : "0"} />
        <input type="hidden" name="workspaceLogoCroppedDataUrl" value={removeLogo ? "" : croppedDataUrl} />

        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          <InitialsAvatar initials={initials} label={label} src={resolvedLogoUrl} size="lg" />
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-[var(--ink)]">Workspace image</p>
            <p className="mt-1 text-xs leading-5 text-[var(--muted)]">
              Upload a JPG, PNG, WEBP, or GIF up to 5 MB. Choose the exact crop before saving so the workspace uses a clean logo everywhere it appears.
            </p>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              >
                {currentLogoUrl || previewUrl ? "Replace image" : "Upload image"}
              </label>
              {(currentLogoUrl || previewUrl) && !removeLogo ? (
                <button
                  type="button"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(15,23,42,0.58)] px-4 py-6 backdrop-blur-sm">
          <div className="w-full max-w-[42rem] rounded-[2rem] border border-[rgba(255,255,255,0.2)] bg-white p-5 shadow-[0_32px_90px_rgba(15,23,42,0.28)] sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--muted)]">Workspace image</p>
                <h3 className="mt-1 text-xl font-semibold tracking-[-0.03em] text-[var(--ink)]">Crop your workspace logo</h3>
                <p className="mt-2 text-sm text-[var(--muted)]">
                  Drag and zoom to choose exactly how your workspace image will look around the app.
                </p>
              </div>
              <button
                type="button"
                ...
                <div>
                  ...
                  <div className="mt-3 flex items-center gap-3 rounded-2xl border border-[var(--line)] bg-white px-3 py-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-xs text-[var(--muted)]">Circular workspace preview</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      setCropError("");
                      try {
                        const nextCroppedDataUrl = await createCroppedWorkspaceLogoDataUrl(cropSourceUrl, croppedAreaPixels);
