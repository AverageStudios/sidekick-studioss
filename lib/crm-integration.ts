...
export async function connectWorkspaceCloseOAuthProvider({
  ...
  const summary = {
    ...
    redirectOrigin: (() => {
      try {
        return new URL(redirectUri || env.closeRedirectUri || "").origin || null;
      } catch {
        return null;
      }
    })(),
  ...
  try {
    ...
    console.error(
      ...
      JSON.stringify({
        ...
        safeErrorDescription: summary.safeErrorDescription,
        failureStage: summary.failureStage,
        requestHost: summary.requestHost,
        redirectOrigin: summary.redirectOrigin,
        usedAttemptNumber:
